...
//				});
			...
			jcoList = new JSONArray();
			//제이슨 배열로 변경
			for(int i=0; i<coList.size(); i++) {
				JSONArray jsubList = new JSONArray();
				for(int j=0; j<coList.get(i).size(); j++) {
					jsubList.add(new JsonReturn().returnPartyCo(coList.get(i).get(j)));
				}
				jcoList.add(jsubList);
			}
		}
