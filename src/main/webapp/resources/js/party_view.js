...
document.addEventListener("DOMContentLoaded", function(){
    ...
    var party_view = new Party_view();
    var myLimit = (limit == null || limit == undefined) ? 11 : limit;
    $(window).on('scroll', function() { 
        if(listCount > (myLimit*(page-1)) && listCount < myLimit*page) {
            if(page != tempPage) {
                if($(window).scrollTop() + $(window).height() + 300 >= $(document).height()) { 
                    party_view.loadMoreData(); 
                    tempPage = page;
                } 
            }
        }
